...
app.get('/hotels/:title', async (req, res)=>{
    try{
        const hotel = await readHotelByName(req.params.title);
        if(hotel){
            res.status(200).json({Hotel: hotel})
        }
        else{
             res.status(400).json({error: "an error occurred while retreiving hotel"})
        }
    }
    catch(error){
        res.status(500).json({error: "an error occurred while retreiving hotel"})
    }
